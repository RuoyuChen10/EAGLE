...
def parse_args():
    parser = argparse.ArgumentParser(description="Gradient baseline attribution for pure LLM prompts.")
    ...
    parser.add_argument(
        "--multiply-by-inputs",
        action="store_true",
        help="Use gradient * embedding before taking the L2 norm.",
    )
    parser.add_argument(
        "--span-reduction",
    ...
    parser.add_argument("--output-dir", default="./gradient_text_attribution_outputs")
...
def main():
    ...
    attributor = GradientTextAttributor(
        qwen.model,
        qwen.tokenizer,
    ...
    json_path = output_dir / "gradient_text_attribution_result.json"
    csv_path = output_dir / "gradient_span_scores.csv"
    html_path = output_dir / "gradient_text_attribution_visualization.html"
    svg_path = output_dir / "gradient_text_saliency_map.svg"
    ...
    print(f"Saved gradient attribution JSON to {json_path}")
    print(f"Saved gradient span CSV to {csv_path}")
    print(f"Saved gradient input visualization HTML to {html_path}")
    print(f"Saved gradient input saliency SVG to {svg_path}")
